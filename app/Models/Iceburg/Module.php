...
class Module extends Model
{
    ...
    public function datalets(){
        return $this->hasMany(Datalet::class, 'module_id', 'id');
    }
                    ...
                    (intval($moduleId) > 0 && $moduleId == $module->id))
                {
                    ...
                    if(!Schema::hasTable($module->name))
                        ...
                        $module->fields()->get()->each(function ($field) use ($module, $table) {
                            ...
                            if(strtolower($field->data_type) == 'datetime')
                            {
                                $table->{$field->data_type}($field->name)->useCurrent();
                            }
